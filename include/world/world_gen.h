...
inline void set_block(WorldData& world, TilePos pos, BlockType block) {
    const size_t index = world.get_tile_index(pos);
    world.blocks[index] = Block(block);
}

inline void remove_block(WorldData& world, TilePos pos) {
    const size_t index = world.get_tile_index(pos);
    world.blocks[index] = tl::nullopt;
}

inline void set_wall(WorldData& world, TilePos pos, const Wall& wall) {
    const size_t index = world.get_tile_index(pos);
    world.walls[index] = wall;
}

inline void remove_wall(WorldData& world, TilePos pos) {
    const size_t index = world.get_tile_index(pos);
...
static WorldData world_generate(uint32_t width, uint32_t height, size_t seed) {
    srand(seed);
