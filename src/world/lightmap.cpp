...
void LightMap::init_area(const WorldData& world, const sge::rect<int64_t>& area, glm::ivec2 tile_offset) {
    ZoneScoped;
    ...
    DoConcurrent(area.height(), [this, area, tile_offset, &world](const std::size_t i) {
        const int64_t y = area.min.y + i;

        for (int64_t x = area.min.x; x < area.max.x; ++x) {
            const LightPos color_pos = LightPos(x, y);
            const glm::ivec2 tile_pos = tile_offset + color_pos / static_cast<int64_t>(SUBDIVISION);
...
uint32_t LightMap::blur_until_black(int64_t start, int64_t stride, glm::vec3& prev_light, float& prev_decay) {
    int index = start;
    uint32_t i = 0;
...
void LightMap::blur_horizontal(const sge::rect<int64_t>& area, const LightMap& reference, glm::ivec2 reference_offset) {
    SGE_ASSERT((area.min.x + area.width()) <= width);
    SGE_ASSERT((area.min.y + area.height()) <= height);
...
void LightMap::blur_vertical(const sge::rect<int64_t>& area, const LightMap& reference, glm::ivec2 reference_offset) {
    SGE_ASSERT((area.min.x + area.width()) <= width);
    SGE_ASSERT((area.min.y + area.height()) <= height);
