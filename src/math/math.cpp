...
glm::vec2 random_point_cone(glm::vec2 direction, float angle, float radius) {
    ...
    const float theta = sge::Random::Float(start_angle, end_angle);
    const float x = glm::cos(theta);
    const float y = glm::sin(theta);

    return glm::vec2(x, y) * sge::Random::Float(0.0f, radius);
}
...
glm::vec2 random_point_circle(float xradius, float yradius) {
    ...
    const glm::vec2 radius = glm::vec2(xradius, yradius) * glm::sqrt(sge::Random::Float(0.0f, 1.0f));
    const float theta = sge::Random::Float(0.0f, 1.0f) * 2.0f * sge::consts::PI;
